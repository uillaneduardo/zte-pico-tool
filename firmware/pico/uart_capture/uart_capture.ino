/*
 * zte-pico-tool — H3601P Passive UART Capture v0.4.0
 *
 * Passive UART byte capture using Arduino-Pico SerialPIO RX only.
 * The Pico never transmits to the ZTE target pins.
 *
 * v0.4.0 adds a framed USB-Capture stream so the host can store bytes
 * continuously instead of relying on the Pico's finite capture buffer.
 *
...
void writeFrame(uint8_t channel, uint8_t *data, uint16_t length) {
...
void streamContinuous() {
  ...
  // Everything after this marker is a binary framed stream until stopped.
  Serial.println("ZTE-CAPTURE-V1");
  ...
  while (true) {
    ...
    if (gp2Count > 0) {
      writeFrame(CHANNEL_GP2, gp2Buffer, gp2Count);
    }

    uint16_t gp3Count = drainToBuffer(pad3Uart, gp3Buffer, STREAM_CHUNK_SIZE);
    if (gp3Count > 0) {
      writeFrame(CHANNEL_GP3, gp3Buffer, gp3Count);
    }
...
void printState() {
  Serial.println();
  Serial.println("=== ZTE Pico Passive UART Capture 0.4.0 ===");
...
void printHelp() {
  Serial.println();
  Serial.println("zte-pico-tool / Passive UART Capture v0.4.0");
  ...
  Serial.println("Capture protocol: ZTE-CAPTURE-V1 + framed binary records.");
